...
internal sealed class XQueryAstBuilder : XQueryParserBaseVisitor<XQueryExpression>
{
    ...
    public override XQueryExpression VisitModule(XQueryParserType.ModuleContext context)
    {
        ...
        if (libraryModule != null)
        {
            ...
            if (prolog != null)
            {
                ...
                // Decimal-format declarations in library modules are module-private but
                // must still be emitted into the module AST so the static analyzer can
                // re-key them to EQName form and propagate them to the importing module
                // (so format-number(..., "lib:euro") called from inside lib:* functions resolves).
                ProcessDecimalFormatDecls(prolog, declarations);
    ...
    /// <summary>
    /// Parses every `declare decimal-format ...` declaration in the prolog and appends
    /// matching DecimalFormatDeclarationExpression entries to <paramref name="declarations"/>.
    /// Shared by main-module and library-module prolog processing so library modules can
    /// export module-private decimal formats (re-keyed to EQName during static analysis).
    /// </summary>
    private static void ProcessDecimalFormatDecls(XQueryParserType.PrologContext prolog, List<XQueryExpression> declarations)
    {
        ...
        foreach (var dfDecl in prolog.decimalFormatDecl())
        {
            ...
            else if (!seenDecimalFormatNames.Add(name))
            {
