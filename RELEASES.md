...
- **Parser recursion-depth bound.** The ANTLR recursive-descent parser (and the AST visitor over its tree) recurse per grammar rule; a query with thousands of nested sub-expressions is now rejected with a catchable parse error past a conservative depth, instead of exhausting the native call stack.
- **Character references to non-XML code points now raise `XQST0090`.** A well-formed `&#…;`/`&#x…;` reference to a code point that is a valid Unicode scalar but not a valid XML character (C0 controls except tab/LF/CR, surrogates, `#xFFFE`/`#xFFFF`, `> #x10FFFF`) is a spec error rather than a silently-produced illegal character. Valid references are unchanged.
- **Namespace-id interning under concurrency.** `XdmDocumentStore.ResolveNamespace` used the shared counter non-atomically, so two threads interning distinct namespace URIs on a shared store could receive the same id; it now uses the atomic increment's own result, so ids stay unique.

- **Opt-in XInclude expansion on the file-parsing `fn:doc` path.** Set `XdmDocumentStore.XInclude` (a `PhoenixmlDb.Core.Xml.XIncludeOptions`) to expand `<xi:include>` elements in documents loaded from file/stream/string — via Core's `XIncludeProcessor` — before XDM conversion; a fatal XInclude failure surfaces as `FODC0002`. Off by default; documents already stored as XDM are unaffected (no text-parse step). Resolution is local-file-only unless `AllowRemote` is set; the intermediate parse honours the document's declared encoding and processes DTDs exactly like the normal load path (internal entities expand; external DTD/entities are never fetched — no XXE). Requires PhoenixmlDb.Core with `XIncludeProcessor`.
...
- **A raw `&` is now accepted in a string literal when parsing XPath (opt-in via `XQueryParserFacade.AllowRawAmpersand`).** XPath — the expression language of XSLT — has no entity or character references: an `&amp;` written in a stylesheet is decoded to a literal `&` by the XML parser *before* the XPath text reaches this parser, so a string literal such as `'Special characters$&'` legitimately contains a bare `&`. The lexer previously rejected it (`token recognition error`, XPST0003) because strict XQuery requires `&` to introduce a predefined-entity or character reference. When the host sets `AllowRawAmpersand` (XSLT does; plain XQuery leaves it off and strict behaviour is unchanged), a bare `&` is a literal ampersand and the literal's text is taken verbatim with no entity decoding. Fixes the W3C XSLT `misc/regex` test `regex-070` (modes a–l), whose mode `j` uses `'Special characters$&'`.
